# Labo automatisering met vagrant (Enterprise Linux)
...
Start de VM met de naam `db` op zoals hieronder getoond. Merk op dat dit even kan duren. De eerste keer dat je dit doet wordt er een zgn. "base box" gedownload, een minimale installatie van AlmaLinux 10 die voor elke nieuwe VM zal hergebruikt worden.

```console
> vagrant up db
Bringing machine 'db' up with 'virtualbox' provider...
==> db: Importing base box 'bento/almalinux-10'...
...
...
- Installeer Apache met ondersteuning voor HTTPS en PHP
- Configureer de firewall zodat de webserver bereikbaar is van buitenaf
- Configureer SELinux waar nodig
- Installeer de PHP-pagina uit het [labo Webserver Hardening](../hard/README.md) die een query uitvoert op de database en het resultaat toont op de webpagina.

## Uitbreiding

Op dit moment heb je de webserver geconfigureerd volgens de typische standaardinstellingen. Pas in een volgende iteratie ook alle hardening-instellingen toe die in het [labo Webserver Hardening](../hard/README.md) beschreven staan. Doe dit volledig geautomatiseerd in het provisioning-script van zowel de `db`- als de `web`-VM.
...
- De inhoud van deze Git-repository wordt binnen de VM gemount onder de directory `/vagrant`. Je kan hier gebruik van maken om bestanden te kopiëren naar de VM.
    - We hebben voorzien dat je in de directory `provisioning` voor elke host een subdirectory kan aanmaken met dezelfde naam als de host, waarin je bestanden die je wil kopiëren naar de VM kan plaatsen. De variabele `${PROVISIONING_FILES}` verwijst naar deze directory.
...
- Probeer regelmatig in de webbrowser van je Linux GUI-VM of de website daar kan getoond worden.
- Als je denkt dat je klaar bent, doe je `vagrant destroy web` en `vagrant up web`. Je webserver zou in één keer moeten geïnstalleerd worden met alle nodige configuratiewijzigingen. Zonder verdere manuele handelingen moet je de website zien in de Linux GUI-VM.
